...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(15.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // Rotación suave en Y
		model = glm::scale(model, glm::vec3(3.0f, 0.0f, 3.0f)); // Grande y ancho
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.0f, -2.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-30.0f), glm::normalize(glm::vec3(0.0f, 3.0f, 0.0f))); // Rotación en diagonal
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f)); // Tamaño mediano
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::rotate(model, glm::radians(140.0f), glm::normalize(glm::vec3(0.0f, 5.0f, 0.0f))); // Rotación en diagonal
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f)); // Tamaño mediano
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
